...
class Command(BaseCommand):
    ...
    def handle(self, component_name: str, **options: Any):
        ...
        context = Context({"name": camelCase, "file_name": file_name})
        ...
        if not comp_path.exists():
            comp_path.mkdir()
        self.create_file(
            comp_path.joinpath(comp_path.joinpath(file_name + ".py")), py_file
        )
        for f in ["html", "css", "js"]:
            self.create_file(
                comp_path.joinpath(comp_path.joinpath(file_name + f".{f}")), ""
            )
