...
def _process_dep_declarations(content: bytes, type: RenderType) -> Tuple[bytes, bytes, bytes]:
    ...
    to_load_css_tags = [tag for media in all_medias if media is not None for tag in media.render_css()]  #
    to_load_js_tags = [tag for media in all_medias if media is not None for tag in media.render_js()]  #
