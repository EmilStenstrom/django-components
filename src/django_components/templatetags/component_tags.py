...
def _parse_tag_preprocess(
    ...
    if not isinstance(tag_name_attr.value, TagValue):
        raise TemplateSyntaxError(f"Invalid tag name '{tag_name_attr.value}'")
    ...
    if isinstance(last_token, TagValue) and last_token.serialize() == "/":
        attrs.pop()
        is_inline = True
    else:
